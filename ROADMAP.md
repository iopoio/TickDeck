...
1. **HTTP Session 풀링** — requests.get() 중복 제거 (10~20초 절감)
2. **코드 정리** — import 통일, _clear_cache 공유화, timeout 상수화
3. **언어별 캐시 분리** — 영문 생성 시 한국어 캐시 혼선 방지 ✅

---

## 🟡 중요 — 다음 작업

4. **랜딩 디자인 마무리** — 한국어/영문 2컬럼 히어로 + 세련된 CTA
5. **SSE 끊김 시 job_id 복원** — 페이지 새로고침해도 결과 확인 가능
6. **processing 상태 자동 실패 처리** — 10분 초과 시 토큰 환불
7. **SSH MCP 설정** — 서버 직접 접근 자동화

---

## 🟢 추후 — 수익화 + 확장

8. **토큰 충전 결제** — 토스페이먼츠 연동
9. **카카오 로그인** — 한국 사용자 확대
10. **슬라이드 수 조절** — 5/8/12장 선택
11. **히스토리 공유 링크** — URL로 결과 공유
