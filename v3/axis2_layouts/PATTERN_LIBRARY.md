...
- **배치 6 (7/23~24 · 제미나이 자기해설 인터뷰 + 실물 2건 대조 — 13p 기획자 리포트·11p 예시덱)**: 후추님 실험 — 제미나이에게 "PPT 비법" 인터뷰(2라운드) 후 실물 대조. **핵심 발견 = 사후 해설은 절반이 지어낸 것**(22p·페이지번호·게이지·비포애프터 등 실물 부재), **반면 규칙을 컨텍스트에 넣고 생성시킨 2호 예시덱은 15문 중 13 적용·코드 대조 일치** → "룰은 사후 회고가 아니라 생성 컨텍스트에 있어야 지켜진다" = designer.md 0단계·계약 코드화 사상의 3차 외부 실증. 원문·실물 = `.claude/research/gemini_ppt_selfrules_2026-07/`. 등재:
  - **WR-no_restatement** ⬜ (writer 룰 후보 1순위) — 차트/표가 이미 보여주는 수치를 캡션·본문·빅넘버로 재서술 금지, 텍스트는 차트 밖 맥락만. 제미나이가 룰로 말하고 **실물 2건 모두 스스로 위반**(13p 3p 캡션 57/43 재언급·예시덱 8p 표 88%+빅넘버 88% 중복) = 반면교사 실증. 채택 근거는 자명성
  - **CL-chunk_cap_3** ⬜ — 페이지당 독립 정보 덩어리 ≤3 (실물 2건 관찰·기존 밀도 계약과 수치 정합 대조 후 병합)
  - **TY-headline_control** ⬜ — 헤드라인 너비 ≤80%·2줄 방어·형태소 단위 줄바꿈 (한국어 auto-fit 레버 후보·발화+예시덱 관찰)
  - **PG-page_number_nav** ⬜ (관찰 1 — 예시덱만) — zero-padded "NN/NN" 우하단 러닝 넘버
  - 재확증: 표 무테(세로선 0·가로선만 — 7/23 표 dense 합치)·간지 초저밀도·무채+단일 액센트·빅넘버 단독 중앙·좌측 마진 일렬 그리드 + **명암 패널+세로 자간 워드마크 관찰 +1** (13p 실물 우측 그라데이션 패널·기존 ⬜ 항목)
  - 기각 보류: 반원 게이지 — 발화 2회·실물 0회 (13p "게이지"라 주장한 85%는 실제론 빅넘버 카드)
  - 부산물 2: ① 제미나이 이미지 소싱 실체 = graphicriver·dribbble·creativemarket 상용 템플릿 참조 (13p 말미 Image Sources placeholder — 외부 제출 시 라이선스 리스크) ② 예시덱 셀프 위반 소소 2건(S5 캡션 중앙 배치 — 자기 룰 "좌하단" 위반·한글 폰트 미지정) = 사전 명세도 완벽 준수는 아님
