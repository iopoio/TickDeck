...
- **배치 7 (7/24 · 코덱스 자기명세 실험 — 배치 6 프로토콜을 코덱스에 적용·클차장 직접 실행)**: `codex exec`로 "규칙 40개 명문화 + 즉시 적용 예시덱 + 자체 검증표 40행" 한 방 생성 (원문 = `.claude/research/codex_deck_selfrules_2026-07/`). **코드 대조 결과 준수율이 제미나이보다 확연히 높음** — CSS 토큰(:root)·12열 그리드·진행바·다크/라임 섹션 전환·순수 CSS 차트(0기준선 막대·conic 도넛·주간 캘린더)·예시 데이터 정직 표기("예시 데이터·n=28일")·미적용 정직 1건(표)·한글 폰트 지정까지. borderline 2건(다크 페이지 색면적 반전·결론부 라임 강조 2곳)만.
  - **독립 수렴 (제미나이 ∩ 코덱스 — 관찰 강화)**: 결론형 제목(제목만 읽어도 서사 성립 — 우리 명제 DAG와 3중 수렴)·문장 2줄 제한(TY-headline_control +1)·페이지당 강조 의미 1개(house 철학)·진행 표시(PG-page_number_nav +1·코덱스는 progress bar 변주)·밀도 완급·장식 배제·좌측 그리드
  - **코덱스 단독 신규 (수치 레버 후보)**: ① **색 면적 배분 60-70/20-30/8-15/3-5** (팔레트의 "무엇"이 아니라 "얼마나") ② **주 블록 면적 55~70% + 열 비율 4:8/5:7/6:6 고정** ③ **차트 상한 세트** — 표 열≤5·막대≤7·선 계열≤3·시점≤12·도넛 범주≤5+합계 100%·**0 기준선**(7/23 블로그 −65% 사고 직결)·범례 대신 직접 라벨 — validator 수치 직수입 후보 1순위 ④ 대비 4.5:1/3:1 + 색+라벨 병행 (접근성) ⑤ 발표 분÷1.0~1.5 = 장수 (intake 레버) ⑥ 연속 3p 동일 구성 → 축 1개 변경 (리듬의 검사화) ⑦ 결론 페이지 = 결론 1+행동 1·새 근거 금지 (writer 룰) ⑧ 5막 스파인(상황→긴장→선택→근거→행동 — 기존 장르 스파인과 대조 후 병합)
  - 메타: 배치 6 결론 재확증 — 명세를 생성 컨텍스트에 두면 준수율이 뛰고, 코덱스처럼 **명세와 시연을 같은 런에서 만들면 정합이 최고**. [미확인] 브라우저 실렌더 (코덱스 sandbox 차단 — 실전 채용 전 렌더 확인 필요)
