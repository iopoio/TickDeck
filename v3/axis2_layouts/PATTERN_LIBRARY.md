...
- **INFO GRAPHIC 160p (상용 팩)**: 순환 화살표·수렴 화살표(Goal)·타임라인 다이어그램 부품 모음 — 기존 다이어그램 문법과 대부분 중복이라 개별 등재 없음. 원본 = `문서_정리/PPTX/f0383680.pptx` (필요 시 부품 사전으로만 참조)
- **폰트 소실 고지**: 환전지갑 84p는 변환 시 임베디드 폰트 소실로 손글씨 fallback 렌더 — 골격(티얼 밴드 표지+라인 일러스트·다운워드 셰브론 카드·브레드크럼 헤더·듀얼 로고 푸터)만 관찰, 타이포 판단은 하지 않음(원본 PPTX 확인 전)
