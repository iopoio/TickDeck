...
- **INFO GRAPHIC 160p (상용 팩 — 7/23 2차 표본 48/160장 후 정정)**: 1차 8장만 보고 "대부분 중복"이라 했던 판단을 정정 — 구간 샘플(40~59·120~139) 추가 열람 결과 신규 3건 등재. ① **CH-hierarchy_pyramid** ⬜ — 계층 피라미드/역피라미드(깔때기)/동심원 스택: 팩 안에서만 7+ 변주(3D·플랫·튜브·동심원) = 화법 자체가 수렴 문법이라는 증거. 현 엔진에 계층 화법 없음 ② **CH-quadrant_matrix** ⬜ — 축 화살표 2×2 사분면(각 상한 색면+아이콘+외부 라벨). **SPC 활용사례 보고서(동구밭 건)가 실전에서 같은 2×2를 사용 — 독립 출처 2 재확증**·컨설팅 캐논 ③ **DC-leader_line_callout** ⬜ — 다이어그램 요소→가는 리더라인→아이콘 배지+라벨 콜아웃(팩 전체 관통·TAETEA·CSES에도 변형 존재 = 관찰 3). **avoid 기록**: 3D 스큐어모피즘(기어·튜브·큐브 입체)·사물 메타포 차트(온도계·플라스크·두상 실루엣) — 2015년대 유행이자 현 하우스 취향(플랫·톤다운)과 정반대, 흡수 금지 목록. 원본 = `문서_정리/PPTX/f0383680.pptx`
