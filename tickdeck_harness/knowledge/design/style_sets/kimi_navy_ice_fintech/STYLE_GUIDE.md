...
**패턴명**: 60/30/10 3색 면적 배분 — 큰 면 딥 컬러 + 비비드 강조 1색 + 라이트 카드

**유지할 것** (색을 바꿔도 이 관계는 그대로):
- 면적 배분 — 60% 큰 면 딥 컬러 · 30% 비비드 강조 · 10% 라이트 카드. 비비드 강조는 1색만
- 텍스트 2색 규율 — on-딥 = 흰색 계열 / on-라이트 = 딥 컬러 재사용. 위계는 제3색이 아니라 opacity 100/70/40 (레버 7)
- 대비 WCAG AA 4.5:1 이상 — 팔레트를 바꿔도 이 검증은 통과해야
- 정직 각주 카드 = 라이트 카드색 배경 + 옅은 보더, 다크 면에서는 강조색 틴트 (rgba .07/.3) — 강조색을 바꾸면 틴트도 따라간다
- 한 덱 한 팔레트 — 혼용 금지

**위 "디자인 토큰"의 hex는 예시 기본값** — 브랜드 팔레트로 교체 가능. 대체 후보 = 토큰 블록의 교체 팔레트 3종(CREATIVE_BRAND·TECH_INNOVATION·SOCIAL_IMPACT). 브랜드 자체 팔레트도 위 배분·대비 규율만 지키면 자유 교체.
