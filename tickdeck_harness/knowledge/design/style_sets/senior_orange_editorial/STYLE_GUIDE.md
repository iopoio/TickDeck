...
**패턴명**: 라이트 틴트 지면 + 단일 액센트 5단 톤스케일 (비교군 무채색)

**유지할 것** (색을 바꿔도 지켜야 하는 관계):
- 액센트는 **1색 계열만** — 두 번째 유채색 도입 금지
- 데이터 톤스케일은 같은 hue **5단** (아주 옅은 틴트 → 본색 → 딥) — 단계가 강도·순위를 인코딩. 딥은 액센트보다 확실히 어둡게 (진한 강조 겸용)
- 지면은 라이트 — 옅은 뉴트럴 틴트 + 흰 카드 float (순백 전면 금지). 다크/액센트 풀블리드는 덱당 1~2장
- 액센트 자리 = 킥커·강조어(hl)·데이터·검증 칩만. 비교군 데이터는 항상 무채색 그레이
- 잉크·서브·푸터 그레이는 액센트와 같은 온도의 뉴트럴 (웜 액센트면 웜 그레이)

**아래 hex는 예시 기본값** — 다른 브랜드/주제 덱에선 위 관계를 지키고 실제 색상만 브랜드 팔레트로 교체:
- 브랜드가 블루면: 단일 블루 액센트로 같은 5단 규칙 — #EAF0FF → #C9D8FF → #7FA3F0 → #2B5FD9 → #16367F
- 브랜드가 그린이면: #E6F4EA → #C2E5CD → #6FBF8B → #1F8A4C → #0F5630
