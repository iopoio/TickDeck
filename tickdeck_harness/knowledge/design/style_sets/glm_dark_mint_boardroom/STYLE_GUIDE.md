...
**패턴명**: 다크 베이스 + 주 액센트(데이터 기본) / 승격 액센트(1위·전환점) 이중 액센트

**유지할 것** (색을 바꿔도 이 관계는 그대로):
- 풀다크 단일 무드 — 베이스 명도 극저, base 2단 + panel 2단 톤스케일
- 액센트 2개 역할 분리 — 주 액센트 = 차트·데이터 기본, 승격 액센트 = 최상위 항목·전환점 1개만. 동시 남발 금지 (레버 3)
- 보조 시그널 2개 한정 — 장기/미래용 쿨톤 1 + 리스크/함정 전용 웜톤 1 (그 외 경고색 금지)
- 텍스트 3단(주/muted/muted2) · 라인은 흰색 알파 .08/.14 헤어라인 저대비
- 정직 각주 카드 = 승격 액센트의 옅은 틴트 (rgba 배경 .06 · 보더 .3) — 승격색을 바꾸면 각주 틴트도 따라간다

**위 "디자인 토큰"의 hex는 예시 기본값** — 브랜드 팔레트로 교체 가능. 대체 후보:
- 딥 퍼플 + 라벤더/골드: base #14102A · panel #1D1740 · 주 #A78BFA · 승격 #F0C674
- 차콜 + 스카이/코랄: base #101418 · panel #1A222B · 주 #67B7E8 · 승격 #E8896B
- 딥 그린 + 라임민트/샴페인: base #0A1F18 · panel #123129 · 주 #6FE3A5 · 승격 #EAD9A0
