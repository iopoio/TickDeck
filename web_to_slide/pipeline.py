...
from .color_resolver import determine_brand_colors
...
기존 body를 포함한 완성된 body 배열을 반환하세요."""
    ...
    # ── primaryColor + accentColor2 결정 (color_resolver.py) ──
    determine_brand_colors(slide_json, assets, url, brand_color, _p)
    _p(f"  → 슬라이드 수: {len(slide_json.get('slides', []))}")
