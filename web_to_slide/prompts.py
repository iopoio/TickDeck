...
## 10. 업종 특화 정보 (해당 시)
다음 업종에 해당하는 경우 아래 정보를 반드시 추출하세요.
- 헬스케어: 임상 근거, 인허가 현황, 환자 성과
- 핀테크: 보안/컴플라이언스, 감사 인증, 거래 규모
- 건설: 인증/안전, 시공 역량, 프로젝트 레퍼런스
...
A (Tech·SaaS·디지털솔루션·에이전시):
  → SaaS, IT컨설팅, AI/데이터 플랫폼, 마케팅에이전시, 핀테크(기본), 헬스케어(B2B), 에듀테크
  → 판단 신호: 소프트웨어/플랫폼 판매, 디지털 서비스 제공, 프로젝트 납품, 솔루션 제공
  → 예) B2B SaaS, 마케팅대행사, AI솔루션, 핀테크 솔루션, B2B 헬스케어 솔루션

B (제조·인프라·엔터프라이즈):
  → 제조, 건설, 중공업, 에너지, 대형 SI, 물류(자산 기반)
  → 판단 신호: 공장·설비·건설 현장, 수주/도급, 자산 집약적 사업, 대규모 B2G
  → 예) 자동화 제조, 플랜트 시공, 통신 인프라, 반도체 장비, 종합건설
...
F (교육·연구·전문서비스):
  → 에듀테크, 교육기관, 연구소, 법무/회계/컨설팅, 채용/HR
  → 판단 신호: 지식·전문성·자격이 핵심 상품, 커리큘럼·방법론·인증이 주요 근거
  → 예) 기업교육, 경영컨설팅, 채용대행

혼재 시:
  B2B SaaS + 크리에이티브 → A
  핀테크 → E(플랫폼) 요소가 강하지 않으면 기본 A
  헬스케어 → B2B 솔루션이면 A, B2C 제품/서비스면 D (F타입 배제)
  건설/시공/건축 → B
  판단 불가 → A
...
[업종별 ⚠️ 특화 슬라이드 변형 규칙]
해당 업종이라 판단되면 아래에 지정된 주제(topic)의 슬라이드를 전체 흐름 내 적절한 위치에 추가/대체하세요.
- 헬스케어 업종 (TYPE A 또는 D): '임상 근거', '인허가 현황', '환자 성과(혹은 도입 병원 성과)' 내용을 다루는 슬라이드 반드시 포함 (예: proof_results 등 활용)
- 핀테크 업종 (TYPE A 기본): '보안/컴플라이언스', '감사 인증', '거래 규모' 슬라이드 반드시 포함
- 건설 업종 (TYPE B): '인증/안전', '시공 역량', '프로젝트 레퍼런스' 슬라이드 반드시 포함
