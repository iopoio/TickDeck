...
> ⚠️ **디자인 경로 기본값 (7/28 후추님 확정)**: 쇼케이스·납품 런의 최종 비주얼은 **hybrid-designer v4.5**가 기본이다 — 아래 7단계(designer→render_deck)는 콘텐츠 검증·스펙 확정까지만 쓰고, 최종 지면은 `hybrid_brief.py → 페이지별 자유 HTML(STYLE_GUIDE+DNA 8레버·exemplar=20260727_senior_hybrid) → hybrid_audit.py → hybrid_assemble.py`로 그린다. 옛 경로(render_deck 완제품)는 데모·내부용.
>
> **스타일은 런마다 설계한다 (7/28 후추님 확정 — "이 디자인이 무조건 나오면 안 된다")**: v4.5의 STYLE_GUIDE는 **런마다 새로 작성**한다 — ① 이 덱의 내용·청중·주제 정서에 맞는 팔레트·모티프·타이포를 고르고 ② 흡수 자산(235개 제안서 교훈·design DNA 레버·PLAYD·knowledge/design의 습득 시스템들)에서 재료를 끌어오고 ③ `_variation_ledger.json`을 읽어 최근 런들과 다른 시스템으로 변주한 뒤 자기 런을 append. 시니어 가이드(20260727_senior_hybrid)는 **형식의 정본**(산출물 계약·DNA 레버 구조·콘텐츠 계약)이지 팔레트·모티프의 정본이 아니다. 불변 = 구조 레버(제목 크게·모서리 메타 프레임·주장 자체를 그리기·검증 칩·정직 각주) / 런마다 설계 = 표현 레버(색·모티프·풀블리드 톤·서체 무드). **스타일 세트 라이브러리 (7/28 후추님 — 목표 20종)**: 세트 = STYLE_GUIDE + 정본 페이지 2장(표지·본문). 검증된 세트는 `tickdeck_harness/knowledge/design/style_sets/`에 등재하고, 새 런은 ① 라이브러리에서 내용에 맞는 세트 선택·조정(기본) 또는 ② 새 세트 창작 후 등재. 실전 런마다 세트가 하나씩 늘어나는 구조 — 곳간이 두꺼워질수록 덱 원가 하락. 현재 보유: 12세트(senior_orange_editorial 등 — 전체 목록은 style_sets/ 디렉터리). **색은 세트에 고정이 아니다(7/28 후추님)**: 각 세트가 정의하는 건 색 관계 패턴(액센트 개수·톤스케일 단계·대비 방식)이지 특정 hex가 아니다 — 새 런은 그 덱의 내용·브랜드에 맞는 실제 색상으로 교체하고, 세트의 '색 체계' 섹션에 적힌 유지 규칙만 지킨다. 7/28 마케팅 재실행에서 옛 경로로 흘러 후추님 지적("학습 다 해놓고 왜 옛 디자인이냐") — 입구 문서 미갱신이 원인이라 여기 박는다.
