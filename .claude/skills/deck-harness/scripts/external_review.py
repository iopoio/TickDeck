...
def build_review_prompt(pages: list[dict[str, str]]) -> str:
    ...
    return "\n\n".join(
        [
            ...
            "5. 독자 패널(무자비한 멘토·7/6): 세 독자가 각자의 질문에 답을 얻는가 — 담당자 '나는 월요일에 뭘 하나'(실행·예시·측정), 상사 '결재 올릴 수 있나'(추정 프레임·시점 정합·권고안), 대표 '요약 1장과 돈·우선순위가 있나'. 답 없는 독자가 있으면 페이지를 지목하라.",
            "6. 문체·실무 톤(7/27 신설): 시장·수치·산업을 주어로 물리·신체 은유를 쓴 동사('비어 있다'·'막힌다'·'닿는다'·'성장을 끈다'류), 압축 은유 명사구('몸통'·'판독'류), 구어 축약('다 크는 건 아니다'류)을 찾아라. 임원 보고서에 그대로 못 올리는 문장을 지목하라. 기계 금지목록이 이미 잡는 알려진 표현이 아니라 그 변형·새 사례를 잡는 것이 네 역할이다.",
            "출력 규칙: 페이지 번호를 반드시 명시한다. 최대 15건만 쓴다. 칭찬 금지. 결함이 없으면 정확히 '없음'이라고만 쓴다.",
