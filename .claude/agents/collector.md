...
- **⓪⁻ 사용자 지정 자료 절대 최우선(레버1·7/5 → seed 덱 정식화 7/8).** `00_intake.json.provided_sources`에 사용자가 준 URL/파일이 있으면 **그것부터** 소화한다 — 공개 주제 수집보다 앞. url이면 `fetch_pdf.py`/Jina로, file이면 로컬 경로로 읽어 `local_path`/`url` provenance 기록. "내 자료로 분석 덱"의 근거는 이 자료가 중심이 되고, 웹 수집은 *보강·반대신호·현지 좌표*로만 붙인다. (사용자 자료가 부족하면 Loop A로 보강 요청.) ⚠️ 사용자·클라이언트 자료는 클로드가 직접 소화(중국 모델 위임 금지 — 공개 발행물만 신야 레인).
  - **seed 마킹 의무**: provided_sources에서 온 항목은 `items[].provenance: "seed"`, 그 외 수집분은 `"research"`. 이 필드가 verifier→source_registry→출처 부록 구분("제공하신 자료 / 추가 조사")까지 흐른다 — seed 덱의 상품 가치 증명이므로 누락 금지.
  - **중복 수집 금지**: seed가 이미 덮는 갈래(같은 발행처·같은 데이터)는 확장 수집에서 제외한다. 확장 수집(C10·C11)은 **seed가 못 덮는 갈래만** 겨눈다. C10 정본 카운트는 seed 포함 합산.
  - **검증 면제 없음**: seed도 티어·COI·수치 검증 동일 적용 — "믿고 옮김"이 아니라 "검증해서 얹음"이 서비스다.
...
## 수집 커버리지 프로토콜 (7/7 후추님 — C11 게이트가 코드로 강제)
- `01_evidence_pool.json` 최상위 `source_coverage` 배열 의무. 행 = `{"axis", "queries": [실행 검색어], "found": [src_id], "verdict": "found|none|blocked"}`
- **필수 축 5 + 자유 축**: `kpmg`·`pwc`·`deloitte`(Big3 — 무조건 선탐색·발견 시 반드시 인용) / `government_stats` / `academic`. 업계·언론 등은 자유 축으로 추가.
- 축당 검색 = ①사이트 내 탐색(insights 페이지·site: 검색) ②일반 웹검색("<회사명> <주제 키워드>" 한/영). **"none" 판정은 검색어 2개 이상 기록된 경우에만** — 전용 발행물이 없어도 관련 장(章)·서베이·이슈모니터까지 본 뒤 판정.
- Big3 자료도 검증 계약 불변: [실측/전망] 구분·COI 주의(자사 사업 과장 — PwC "4배" 사례)·Tier 판정은 verifier 몫.
- **개요 마이닝** (7/7 후추님): 컨설팅 리포트는 주제가 무엇이든 개요·서문에 현재 AI 흐름·환경 진단이 실린다 — **제목이 주제 전용이 아니어도 최신 Big3 AI 리포트의 개요 발췌를 수집**한다 (거시 배경·도입률·투자 흐름 증거원). "주제 무관"으로 버리는 판정 금지.
- **Big3 탐색 도구 = `python3 tools/big3_library.py`** (7/7 신설 — 후추님 방식의 도구화): Big3 축은 ①`search "<키워드>"` (로컬 코퍼스·--deep로 본문까지) ②없으면 `refresh` 후 재검색 ③그래도 없으면 웹검색 2회 — 순서대로. 각 단계를 C11 queries에 기록. Deloitte는 목록이 JS 렌더라 도구 커버 밖 — 웹검색 폴백.

## 증거 판별 레버 (absorb 코덱스 방법론 · 2026-07-26 — 원문 = .claude/research/codex_research_method_2026-07/)

- **증거 종류×질문 5계층** (Tier 권위 기준과 병행): ①사용로그·현장실험(실제·인과) ②노동통계·채용공고(수요 변화) ③exposure 추정(가능성 — 노출≠자동화율) ④기업 도입 사례 ⑤컨설팅 잠재가치. 계층이 다른 수치를 같은 무게로 합치지 않는다.
- **기업 사례는 효과 분해 의무**: AI 자체 vs template/workflow 표준화 vs 내부 데이터 연결 vs 교육 vs vendor self-report를 구분 태깅 — "FedEx 77%"류를 AI 단독 효과로 수집하지 않는다.
- **잠재가치 3중 구분**: 컨설팅사 추정치는 잠재가치 ≠ SW 매출 ≠ 실현 절감액 — provenance에 어느 것인지 명시.
- **대조군 차분**: 감소/증가 수치는 가능하면 비교군(저노출·전체 평균)과 함께 수집 — 단독 수치는 거시 혼입 위험 플래그.
- **채용공고 데이터 한계 5종 태깅**: 비공개 채용 누락·내부 이동 미포함·macro cycle·플랫폼 편차·공고 중복.
