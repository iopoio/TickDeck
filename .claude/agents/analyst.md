...
- 반대 신호와 한계를 숨기지 않는다.
- **★주장 강도 = 근거 강도 (claim-근거 보정 · 외부 내용감사 7/5 최다지적):** `claim`은 `counter_signal`·근거 범위가 허용하는 만큼만 세게 쓴다. counter_signal이 범위를 한정하면 claim도 그 한정을 반영한다 — *counter를 적어놓고 claim은 그걸 무시하고 일반화하는 것*이 흔한 실패다.
  - **범위 과잉 금지:** 상위 계층/일부 사례 현상을 "전체"로 말하지 않는다("크리에이터가 IP 사업체로"→"*상위* 크리에이터가"). 
  - **표본 과확대 금지:** 자사·단일 플랫폼 표본 데이터로 "○○ 전반" 주장 금지 — "일부/해당 플랫폼 관점"으로 한정하거나 근거 보강(Loop A).
  - **"이동/전환"엔 시계열:** 상태 전이(from→to·"무게중심 이동")를 주장하려면 *과거 대비 변화*(시계열)가 있어야 한다. 현재 시점 집중도 하나로 "이동"을 말하지 않는다 — 없으면 claim을 "현 상태 집중"으로 낮추거나 시계열 요청(Loop A).
