...
- **split을 기본값으로 쓰지 않는다.** 본문 컴포지션은 내용이 고른다: `stack`(주 비주얼 전폭 상단 + 하단 카드 가로 배열 — 차트가 좌우 칸에 눌리거나 좌우 밸런스가 안 맞을 때), `split`(주/보조 비주얼이 대등하게 병렬일 때), `statement`(단일 메시지 집중), `stepper`(절차·단계). 페이지마다 "왜 이 컴포지션인가" 한 줄 근거를 만들 수 있어야 한다.
