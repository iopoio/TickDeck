...
5. **밀도·리듬:** 텍스트 빽빽한 장만 잇지 않는다. 4~5장마다 *숨 쉬는 장*(목차·패턴 요약·여백)을 둔다. **모든 파트는 간지로 연다 — 1장짜리 파트도 예외 없음**(후추님 7/2: "한 장인 건 알지만 간지 없이 바로 내용이 나오니 어색"). 간지에는 part_index/part_label/part_count를 명시하고 하위 목차 프리뷰 bullets(해당 파트 short_title 그대로·숫자 든 제목 제외)를 내린다.
