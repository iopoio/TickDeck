...
- **간지에 다음 페이지 제목 나열 금지** (7/7 후추님): 간지 = 파트 아우르는 제목 (+선택적 한 줄)뿐. 미리보기 불릿은 칩 내비·본문 제목과 3중 중복이고 실전 문서(BOND·KPMG) 관찰 0인 장치.

## 밀도 2겹 룰 (7/8 신설 — 납품·쇼케이스 런 한정 · "정직한데 얇다"의 구조 해법)

- 덱의 **핵심 주장 페이지**(diagnosis·scenario·convergence 역할)는 `allowed_metric_ids`/`allowed_source_ids`에 **독립 출처 2개 이상**을 내리고, 가능하면 **실명 사례 1개**(기업명·날짜·숫자 있는 서사)를 message에 포함시킨다. 페이지에 다 노출하란 뜻이 아니라(추림 두 칼은 그대로), designer가 고를 **재료의 하한**이다.
- 레지스트리에 2겹을 채울 재료가 없으면 그 주장을 약화하지 말고 **Loop A로 보강 요청**한다 — 얇게 내보내는 것보다 재수집이 싸다.
- 좋은 밀도의 모양: "A 기관은 2024년 X%, B 기관은 2025년 Y%로 상승 전망" — 단일 인용이 아니라 교차·시계열 배치 (7/8 경쟁분석 수렴 항목).
- 데모·내부 런은 면제 (비용 절약).

## 논증 도식 선택 룰 (7/8 R5 신설 — DESIGN_R5_argument_diagrams.md)

주장 페이지에서 논증 구조가 있으면 텍스트 블록+범용 차트보다 **argument family 도식을 우선 고려**한다:
- 주장 1개 + 근거 2겹 이상 (밀도 2겹 룰 충족 페이지) → `pyramid` — 근거 겹이 그림으로 보임
- "A가 B를 낳고 B가 C를 낳는다" 인과 서사 → `causal_chain` — 링크마다 근거 캡션
- 플레이어/선택지 포지셔닝 (market-research 장르 필수 검토) → `two_by_two` — 축은 논증의 실제 변수로
- 대립·양자택일 (ARCH-versus 궁합) → `tradeoff`
단, **실제 논증 구조가 spec에 있을 때만** — 구조 없는 페이지에 도식을 장식으로 넣지 않는다. 슬롯 최소(근거≥2·노드≥3·아이템≥3) 못 채우면 쓰지 않는다.


## 출처 부록 룰 (7/8 후추님 — "중요하지 않은 출처는 지워")

- 부록 `allowed_source_ids` = **덱 본문·차트가 실제 인용하는 출처만** (렌더러 `_deck_cited_source_ids` 기준). 수집·허용 전체를 덤프하지 않는다 — 미인용 출처는 registry(URL 포함)에 보존되어 팩트체크·감사 추적은 유지된다.
- 페이지 수 = 렌더러 2단 기준(짧은 출처 ~40행/장·긴 복합 인용은 8~9행) 안에서 최소로. 인용 20여 건이면 1장.
